...
class Vehicle:
    ...
    def get_data(self):
        return (self.get_stock_number(), self.vin, self.make, self.model, self.year, self.odo, self.purchase_price, self.calculate_repairs(), self.total_cost(), self.status)

    def __repr__(self):
        return f"""
STOCK #         : {self.get_stock_number()}
VIN             : {self.vin}
MAKE            : {self.make}
MODEL           : {self.model}
YEAR            : {self.year}
